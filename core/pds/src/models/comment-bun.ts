/**
 * Comment repository - Bun SQLite version
 */

import { Database } from 'bun:sqlite';
import type { DatabaseConnection } from '../database/bun-connection';
...
export class CommentRepository {
  ...
  findByParent(parentId: string, limit = 50): Comment[] {
    const stmt = this.db.prepare('SELECT * FROM comments WHERE parent_id = ? ORDER BY created_at DESC LIMIT ?');
    const rows = stmt.all(parentId, limit) as any[];
    return rows.map((row) => this.rowToComment(row));
  }

  findByDID(did: string, limit = 50, offset = 0): Comment[] {
    const stmt = this.db.prepare('SELECT * FROM comments WHERE did = ? ORDER BY created_at DESC LIMIT ? OFFSET ?');
