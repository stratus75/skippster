/**
 * Video repository - Bun SQLite version
 */
...
export class VideoRepository {
  ...
  findAll(limit = 50, offset = 0): Video[] {
    const stmt = this.db.prepare('SELECT * FROM videos ORDER BY created_at DESC LIMIT ? OFFSET ?');
    const rows = stmt.all(limit, offset) as any[];
    return rows.map((row) => this.rowToVideo(row));
  }

  findByDID(did: string, limit = 50, offset = 0): Video[] {
    const stmt = this.db.prepare('SELECT * FROM videos WHERE did = ? ORDER BY created_at DESC LIMIT ? OFFSET ?');
